...
仓库随附 Skill 实际使用的处理后知识库：`kb/manifest.json` 登记 18 部典籍的
稳定身份、处理后 Markdown 路径和 SHA-256，`kb/texts/` 保存分书、统一编码、
补入结构标题与稳定锚点后的唯一运行正文，`kb/assets/辅行诀/` 保存汤液图与经法
规则等结构化资产。原始下载快照和历史合并源文件不进入运行包，也不是第二套
知识库真源。具体结构见 [`kb/README.md`](kb/README.md)。

这些文件是完整安装的一部分。`manifest.json`、登记正文或结构化资产缺失、被
替换或哈希不符时，正式运行应停止，不能降级成“无典籍但已完整安装”。检索仍只
在需要核对直接原文、出处、调用权、反面证据或决定性边界时提供少量短原文；不把
整章或整部典籍灌入模型上下文。
...
该命令强制校验随仓库提供的 18 部处理后正文、5 个结构化资产、来源哈希、检索
回读、汤液规则和直接来源测试；缺少知识库不会被判为通过。也可以单独执行：
...
软件代码、原创文档和仓库内示例图采用 [MIT License](LICENSE)。随仓库提供的
第三方数字典籍仍保留各自来源与许可身份，不因本软件许可证而被重新授权，详见
